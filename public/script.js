document.addEventListener('DOMContentLoaded', () => {
    ...
    async function updateLeaderboard(name, newScore) { 
        ...
        if (isNaN(scoreNumber)) {
            console.error("Invalid score submitted:", newScore);
            return;
        }
        try { 
            await db.collection("leaderboard").add({ name: name, score: scoreNumber, createdAt: firebase.firestore.FieldValue.serverTimestamp() }); 
            console.log("Score submitted:", name, scoreNumber); 
        } catch (error) { 
