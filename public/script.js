document.addEventListener('DOMContentLoaded', () => {
    ...
    async function updateLeaderboard(name, newScore) {
        ...
        try {
            ...
            await listenForLeaderboardChanges(); // 提交成功后立刻刷新一次排行榜
        } catch (error) {
    ...
    async function listenForLeaderboardChanges() {
        try {
            ...
            gameState.cachedLeaderboard = results.map(item => item.attributes);
            console.log("Leaderboard loaded:", gameState.cachedLeaderboard.length);
            // 只要调用就刷新
